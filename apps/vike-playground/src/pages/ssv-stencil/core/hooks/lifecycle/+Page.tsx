	...
	return (
		<div className="flex flex-col gap-6">
			...
			<div className="flex flex-col gap-1">
				<h1 className="text-2xl font-bold text-[var(--color-fg)]">Lifecycle Hooks Explorer</h1>
			...
			<div className="flex items-center gap-3">
				...
				<span
					...
					<span className={`w-1.5 h-1.5 rounded-full ${mounted ? "bg-[#4ade80] animate-pulse" : "bg-[#f87171]"}`} />
			...
			<div className="rounded-xl border border-[var(--color-card-border)] bg-[var(--color-card)] p-5">
				{mounted ? (
					...
					<div className="flex items-center justify-center py-10 text-sm text-[var(--color-muted-fg)] gap-2">
			...
			<div className="flex flex-col gap-3">
				<h2 className="text-sm font-semibold text-[var(--color-fg)]">Hook Reference</h2>
				<div className="rounded-lg border border-[var(--color-border)] overflow-hidden text-xs">
					<table className="w-full">
						<thead>
							<tr className="bg-[var(--color-surface)] border-b border-[var(--color-border)]">
								<th className="text-left px-3 py-2 text-[var(--color-muted-fg)] font-semibold uppercase tracking-wider text-[0.65rem]">
									Hook
								</th>
								<th className="text-left px-3 py-2 text-[var(--color-muted-fg)] font-semibold uppercase tracking-wider text-[0.65rem]">
									Stencil lifecycle
						...
						<tbody className="divide-y divide-[var(--color-border)]">
							{hookRows.map(row => (
								<tr key={row.hook} className="hover:bg-[var(--color-surface)] transition-colors">
									<td className="px-3 py-2">
										<span
											className="inline-block px-2 py-0.5 rounded text-[0.7rem] font-mono font-semibold"
											style={{ background: `${row.color}18`, color: row.color, border: `1px solid ${row.color}40` }}>
											{row.hook}
										</span>
									</td>
									<td className="px-3 py-2 font-mono text-[var(--color-muted-fg)]">{row.stencil}</td>
									<td className="px-3 py-2 text-[var(--color-muted-fg)]">{row.when}</td>
								</tr>
							))}
						</tbody>
					</table>
			...
			<div className="flex flex-col gap-2 text-xs text-[var(--color-muted-fg)]">
				<p>
					Open DevTools and watch the <strong className="text-[var(--color-fg)]">Console</strong> — each hook fires a
					styled <code>console.warn</code> so you can correlate the log entries with the timing in the browser.
				</p>
				<p>
					Click <strong className="text-[var(--color-fg)]">Force Re-render</strong> inside the component to trigger an
					update cycle and see <code>hostWillUpdate</code>, <code>hostWillRender</code>, <code>hostDidRender</code>, and{" "}
					<code>hostDidUpdate</code> fire. Notice that <code>hostWillLoad</code> and <code>hostDidLoad</code> only fire
					once — on first load.
				</p>
				<p>
					Click <strong className="text-[var(--color-fg)]">Unmount component</strong> above to remove the element from
					the DOM and see <code>hostDisconnected</code>. Re-mounting fires <code>hostConnected</code> again, followed by
					a fresh <code>hostWillLoad</code> / <code>hostDidLoad</code> sequence.
				</p>
			</div>
		</div>
	);
