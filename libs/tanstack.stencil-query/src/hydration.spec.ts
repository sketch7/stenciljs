...
describe("hydration", () => {
	it("dehydrate captures query state", async () => {
		...
		await qc.prefetchQuery({ queryKey: ["posts"], queryFn: async () => [{ id: 1, title: "Hello" }] });
	...
	it("hydrate restores query state into a fresh client", async () => {
		...
		await source.prefetchQuery({ queryKey: ["posts"], queryFn: async () => [{ id: 1 }] });
	...
	it("hydrated client returns data immediately without fetch in useQuery", async () => {
		...
		using m = await mount(() => ({
			query: useQuery({ queryKey: ["posts"], queryFn: async () => [], staleTime: Infinity }, target),
		}));
	...
	it("hydrate merges into existing cache without overwriting fresh data", async () => {
		const source = new QueryClient();
		await source.prefetchQuery({ queryKey: ["item"], queryFn: async () => "stale" });
		const state = dehydrate(source);
		source.clear();
